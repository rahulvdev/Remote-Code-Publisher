...
void CodeAnalysisExecutive::startPublish(std::string category) {
	std::cout << "--------------------------------------------------------------------------------------------------------\n";
	...
	try {
		...
		complexityAnalysis();
		std::vector<std::string> allFiles = getAllsubFiles();
		//Start Type Analysis
		TypeAnalysis tb;
		tb.startTypeAnalysis();
		//Start Dependancy Analysis
		DependancyAnalysis dp(depXmlPath, 50);
		...
		CodePublisher cPub(dp,category);
		cPub.fileCollector(allFiles, lineMapInst,category);
	}
